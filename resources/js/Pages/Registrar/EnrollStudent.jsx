...
export default function EnrollStudent({
  ...
  const [formData, setFormData] = useState({
    assigned_strand_id: canReEnrollToSameStrand && latestEnrollment?.assigned_strand?.id 
      ? latestEnrollment.assigned_strand.id.toString() 
      : '',
    assigned_section_id: canReEnrollToSameStrand && latestEnrollment?.assigned_section?.id
  ...
  useEffect(() => {
    if (formData.assigned_section_id && activeSchoolYear && activeSemester) {
      ...
      axios.get(`/api/sections/${formData.assigned_section_id}/schedule-preview`, {
        params: {
  ...
  const handleSubmit = (e) => {
    ...
    setProcessing(true)
    // Note: We'll need to update the backend to return enrollment ID instead of redirecting
    router.post(
      '/registrar/enrollments/re-enroll-auto',
      {
      ...
      {
        onSuccess: (page) => {
          // Reload the page to show the COR
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-hidden">
          <div className="h-full flex">
            ...
            {/* Enrollment Form - Hidden if already enrolled */}
            {!currentTermEnrollment && (
                        ...
                        {Object.entries(
                          ...
                          <div key={day} className="border-l-4 border-indigo-500 pl-4">
                            ...
                            <div className="space-y-2">
                              {classes.sort((a, b) => a.start_time.localeCompare(b.start_time)).map((cls, idx) => (
