...
export default function Profile({ faculty, user, flash = {} }) {
  ...
  }, [faculty, user, isEditing, setData])
  ...
  const handleSubmit = (e) => {
    ...
    post('/faculty/profile', {
      ...
      onSuccess: (page) => {
        setIsEditing(false)
        reset('profile_photo')
        const updated = page.props.faculty || page.props.user
        if (updated) {
          setData({
  ...
  const breadcrumbItems = [
    ...
    { label: 'Profile', href: '/faculty/profile', current: true }
  ]
                ...
                <div className="p-6 space-y-8">
                  <div className="flex items-center space-x-6">
                    <div className="w-24 h-24 bg-indigo-100 rounded-full flex items-center justify-center overflow-hidden">
                      {profilePhoto ? (
                        <img src={`/storage/${profilePhoto}`} alt="Profile" className="w-full h-full object-cover" />
                      ) : (
                    ...
                    <div>
                      ...
                      <div className="mt-2 flex flex-wrap gap-2">
                        ...
                        {faculty?.assigned_strand?.Strand_name && (
                  ...
                  {user?.is_coordinator && (
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          <h3 className="text-sm font-semibold text-blue-900">Coordinator Status</h3>
                          <p className="text-sm text-blue-700 mt-1">
                            You have coordinator privileges with access to enrollment workflows.
                          </p>
                        </div>
                      </div>
                    </div>
                  ...
                  {isEditing ? (
                    <form onSubmit={handleSubmit} className="space-y-6">
                      <div>
                        ...
                        <p className="mt-1 text-sm text-gray-500">Maximum file size: 10MB. Supported formats: JPEG, PNG, JPG, GIF</p>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          ...
                          <p className="text-sm text-gray-900">{user?.is_coordinator ? 'Coordinator' : 'Faculty'}</p>
                        </div>
                        <div>
                          <label className="block text-sm font-medium text-gray-700 mb-2">Assigned Strand</label>
                          <p className="text-sm text-gray-900">{faculty?.assigned_strand?.Strand_name || 'Not assigned'}</p>
                        </div>
                      </div>
                    ...
                    <div className="space-y-6">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <p className="mt-1 text-sm text-gray-900">{user?.is_coordinator ? 'Coordinator' : 'Faculty'}</p>
                        </div>
                        <div>
                          <label className="block text-sm font-medium text-gray-700">Assigned Strand</label>
                          <p className="mt-1 text-sm text-gray-900">{faculty?.assigned_strand?.Strand_name || 'Not assigned'}</p>
                        </div>
                      </div>
                    </div>
                  )}
