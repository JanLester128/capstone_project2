...
export default function StudentSidebar({ enrollmentStatus }) {
  ...
  return (
    <aside className="bg-white border-r border-gray-200">
            ...
            <li key={n.href}>
              {n.disabled ? (
                ...
                >
                  ...
                  {n.status === 'closed' && !isActive(n.href) && <span className="text-[10px] text-red-500">✕</span>}
                  {n.status === 'open' && !isActive(n.href) && <span className="text-[10px] text-green-500">●</span>}
                </Link>
              )}
            </li>
          ))}
      ...
      {/* Mobile flyout */}
      {open && (
        <nav id="student-mobile-nav" className="md:hidden p-4 space-y-1 border-b" role="dialog" aria-modal="true">
          {nav.map((n) => (
            n.disabled ? (
              <div
                key={n.href}
              ...
              <Link
                ...
                className={(isActive(n.href) ? 'bg-indigo-50 text-indigo-700' : 'text-gray-700 hover:bg-gray-50') + ' flex items-center justify-between rounded-md px-3 py-2 text-sm'}
                aria-current={isActive(n.href) ? 'page' : undefined}
              >
                <span>{n.label}</span>
                {n.status === 'closed' && !isActive(n.href) && <span className="text-[10px] text-red-500">✕</span>}
